...
deta_re$age_1_t1 <- ifelse(is.na(deta_re$age_1_t1), deta_re$age_1_t2, deta_re$age_1_t1)
deta_re$age_1_t2 <- ifelse(is.na(deta_re$age_1_t2), deta_re$age_1_t1, deta_re$age_1_t2)
...
for (i in 1:length(CPG_all)) {
  tryCatch({
    ...
    covVars <- c("age_1_t1","age_1_t2","sex_t1","sex_t2") # You can reduce or increase the covariates as you need
    ...
    modelMZ<- mxModel("MZ",
                      ...
                      mxMatrix( type="Full", nrow=ncv, ncol=2, free=F, label=c("data.age_1_t1","data.sex_t1",
                                                                               "data.age_1_t2","data.sex_t2"), name="MZDefVars"),
    ...
    modelDZ<- mxModel("DZ",
                      mxMatrix( type="Full", nrow=ncv, ncol=2, free=F, label=c("data.age_1_t1","data.sex_t1",
                                                                               "data.age_1_t2","data.sex_t2"), name="DZDefVars"),
                      mxMatrix( type="Full", nrow=1, ncol=ncv, free=TRUE, values=0,labels=c("Bage","Bsex"), name="BageTH"),
    ...
    round( SatSumm$CI[,c(2,1,3)] , 3)  #in the order: Estimate, lower bound, upper bound#
      ...
      modelMZ<- mxModel("MZ",
                        mxMatrix( type="Full", nrow=ncv, ncol=2, free=F, label=c("data.age_1_t1","data.sex_t1",
                                                                                 "data.age_1_t2","data.sex_t2"), name="DefVars"),
      ...
      round(AdeSumm$CI[,c(2,1,3)] , 2 )
      
      mxCompare (SatFit,AdeFit)
      #--------------------------------------------------------------------------
      ##### AEmodel  #####
      
      AEmod <- omxSetParameters(model=AdeFit,labels="d11",free=F,values=0,name="AE")
      ...
      mxCompare(base=AdeFit,comparison=AEfit)
      #--------------------------------------------------------------------------
      ##### DEmodel  #####
      
      DEmod <- omxSetParameters(model=AdeFit,labels="a11",free=F,values=0,name="DE")
      DEfit <- mxTryHardOrdinal(DEmod,intervals=T,extraTries = 15)
      (DEsumm <- summary(DEfit,verbose=T))
      DEsumm$CI[4,1] <- DEsumm$CIdetail[7,3]
      DEsumm$CI[5,1] <- DEsumm$CIdetail[9,3]
      DEsumm$CI[6,1] <- DEsumm$CIdetail[11,3]
      DEsumm$CI[4,3] <- DEsumm$CIdetail[8,3]
      DEsumm$CI[5,3] <- DEsumm$CIdetail[10,3]
      DEsumm$CI[6,3] <- DEsumm$CIdetail[12,3]
      
      #CEsumm$CI
      round( DEsumm$CI[,c(2,1,3)] , 2 )
      mxCompare(base=AdeFit,comparison=DEfit)
      #--------------------------------------------------------------------------
      ###### E model    ##########
      
      Emod <- omxSetParameters(model=DEfit,labels="d11",free=F,values=0,name="uniE")
      ...
      mxCompare(base=AdeFit,comparison=Efit)
      mxCompare(base=AdeFit,c(AEfit,DEfit,Efit))
      fitcompare <- mxCompare(base=AdeFit,c(AEfit,Efit))
      mxCompare (SatFit,AdeFit)
      ...
      result[vart,6] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[4,1],
                               ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[4,1],'NA'))
      result[vart,7] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[4,2],
                               ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[4,2],'NA'))
      result[vart,8] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[4,3],
                               ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[4,3],'NA'))
      result[vart,9] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[5,1],
                                      ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE','NA','NA'))
      result[vart,10] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[5,2],
                                       ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE','NA','NA'))
      result[vart,11] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[5,3],
                                       ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE','NA','NA'))
      result[vart,12] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[6,1],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[6,1],round( Esumm$CI[,c(2,1,3)] , 2 )[6,1]))
      result[vart,13] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[6,2],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[6,2],round( Esumm$CI[,c(2,1,3)] , 2 )[6,2]))
      result[vart,14] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[6,1],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[6,1],round( Esumm$CI[,c(2,1,3)] , 2 )[6,3]))
      result[vart,15] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[1,1],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[1,1],'NA'))
      result[vart,16] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[1,2],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[1,2],'NA'))
      result[vart,17] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[1,3],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[1,3],'NA'))
      result[vart,18] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[2,1],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[2,1],'NA'))
      result[vart,19] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[2,2],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[2,2],'NA'))
      result[vart,20] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[2,3],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[2,3],'NA'))
      result[vart,21] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[3,1],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[3,1],round( Esumm$CI[,c(2,1,3)] , 2 )[3,1]))
      result[vart,22] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[3,2],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[3,2],round( Esumm$CI[,c(2,1,3)] , 2 )[3,2]))
      result[vart,23] <- ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),round(AdeSumm$CI[,c(2,1,3)] , 2 )[3,3],
                                ifelse(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]=='AE',round(AEsumm$CI[,c(2,1,3)] , 2 )[3,3],round( Esumm$CI[,c(2,1,3)] , 2 )[3,3]))
 
      result[vart,24] <- AdeSumm$informationCriteria[1,2]
  }, error = function(e) {print(paste("error",i,sep=" "))})
}
write.csv(result,file = '/public/home/hongxming/uni_bmi/results_bmi_longi1_ade.csv')
