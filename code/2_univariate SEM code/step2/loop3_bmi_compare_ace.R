...
for (i in 1:length(CPG_all)) {
  tryCatch({
    ...
    round( SatSumm$CI[,c(2,1,3)] , 3)  #in the order: Estimate, lower bound, upper bound#
      ...
      lon <- omxSetParameters(model=AceFit,labels="e11",free=F,values=sqrt(results_bmi_longi1_acde[vart,'e2']),name="lon")
      ...
      loncompare <- mxCompare(base=AceFit,comparison=lonfit)
      
    lonresult[vart,'P_value'] <- loncompare[2,'p']
  }, error = function(e) {print(paste("error",i,sep=" "))})
}
write.csv(lonresult,file = '/public/home/hongxming/uni_bmi/lonresults_bmi_ace.csv')
