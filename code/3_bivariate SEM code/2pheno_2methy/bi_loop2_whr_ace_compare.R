...
for (i in 1:length(CPG_all)) {
    ...
    dropRc <- omxSetParameters(dropC1,labels=c("rc21"),free=F,values=0,name="dropRc")
    dropRcfit  <- mxTryHard(dropRc, intervals=T, extraTries = 15)
    ...
    fitcompare$comparison[1] <- 'fitACE'
    ...
    loncompare <- mxCompare(base=get(ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),'fitACE',fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))])),comparison=lonfit)
    lonresult[vart,'P_value'] <- loncompare[2,'p']

  }, error = function(e) {print(paste("error",i,sep=" "))})
}
write.csv(lonresult,file = '/public/home/hongxming/bmi_ace/bi_whr_results_2_ace_compare.csv')
