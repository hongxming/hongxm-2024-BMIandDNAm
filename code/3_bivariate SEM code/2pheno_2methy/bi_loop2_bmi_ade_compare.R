...
PHENO_2 <- subset(PHENO_2,select=c(ID20,BMI_2))
deta <- merge(deta,PHENO_2,by.x='ID20_t1',by.y='ID20',all.x=T)
deta <- merge(deta,PHENO_2,by.x='ID20_t2',by.y='ID20',all.x=T)
deta$BMI_t1 <- deta$BMI_2.x
deta$BMI_t2 <- deta$BMI_2.y
deta_re <- deta
load('/public/home/hongxming/bmi_ace/result_frame.Rdata')
bi_bmi_1_result <- read.csv('/public/home/hongxming/bmi_ace/bi_bmi_1_result_ade.csv')
rownames(bi_bmi_1_result) <- bi_bmi_1_result$CPG
...
for (i in 1:length(CPG_all)) {
    ...
    fitcompare <- fitcompare[-1,]
    lon <- omxSetParameters(model=get(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),labels="ra21",free=F,values=bi_bmi_1_result[vart,'Ra'],name="lon")
    lon <- omxSetParameters(model=get(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),labels="re21",free=F,values=bi_bmi_1_result[vart,'Re'],name="lon")
...
write.csv(lonresult,file = '/public/home/hongxming/bmi_ace/bi_bmi_results_2_ade_compare.csv')
