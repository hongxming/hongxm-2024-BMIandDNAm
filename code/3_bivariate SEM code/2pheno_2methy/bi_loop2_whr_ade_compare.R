...
mean1 <- c()  #mean sbp
mean2 <- c()  #mean dbp
for (i in 1:nrow(PHENO_2)){
  if(is.na(PHENO_2$WAI1_1[i]) == T){
    mean1 <- c(mean1, NA)
  }else{
    d <- c(PHENO_2$WAI1_1[i],PHENO_2$WAI2_1[i],PHENO_2$WAI3_1[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
    if(c == 0){
      mean1 <- c(mean1, mean(c(b,a)))
    }else{
      if(c < mean(c(a,b))) {
        mean1 <- c(mean1, mean(c(b,c)))
      }else{
        mean1 <- c(mean1, mean(c(a,c)))
      }
    }### Ñ¡È¡Ïà½üµÄÁ½¸öÖµµÄ¾ùÖµ×÷ÎªÑªÑ¹µÄ×îÖÕÖµ
  }
  if(is.na(PHENO_2$WAI1_2[i]) == T){
    mean2 <- c(mean2, NA)
  }else{
    d <- c(PHENO_2$WAI1_2[i], PHENO_2$WAI2_2[i], PHENO_2$WAI3_2[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
    if( c== 0){
      mean2 <- c(mean2, mean(c(b,a)))
    }else{
      if(c < mean(c(a,b))) {
        mean2 <- c(mean2, mean(c(b,c)))
      }else{
        mean2 <- c(mean2, mean(c(a,c)))
      }
    }
  }
}
...
mean2 <- c()  #mean dbp
for (i in 1:nrow(PHENO_2)){
  if(is.na(PHENO_2$HIP1_1[i]) == T){
    mean1 <- c(mean1, NA)
  }else{
    d <- c(PHENO_2$HIP1_1[i],PHENO_2$HIP2_1[i],PHENO_2$HIP3_1[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
  ...
  if(is.na(PHENO_2$HIP1_2[i]) == T){
    mean2 <- c(mean2, NA)
  }else{
    d <- c(PHENO_2$HIP1_2[i], PHENO_2$HIP2_2[i], PHENO_2$HIP3_2[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
...
PHENO_2$HIP_1 <- mean1
PHENO_2$HIP_2 <- mean2
PHENO_2$WHR_1 <- PHENO_2$WAI_1/PHENO_2$HIP_1
PHENO_2$WHR_2 <- PHENO_2$WAI_2/PHENO_2$HIP_2
PHENO_2 <- subset(PHENO_2,select = c(ID20,WHR_2))
deta <- merge(deta,PHENO_2,by.x='ID20_t1',by.y='ID20',all.x=T)
deta <- merge(deta,PHENO_2,by.x='ID20_t2',by.y='ID20',all.x=T)
deta$WHR_t1 <- deta$WHR_2.x
deta$WHR_t2 <- deta$WHR_2.y
deta_re <- deta
load('/public/home/hongxming/bmi_ace/result_frame.Rdata')
bi_whr_1_result <- read.csv('/public/home/hongxming/bmi_ace/bi_whr_1_result.csv')
rownames(bi_whr_1_result) <- bi_whr_1_result$CPG
ACESummer_re <- ACESummer
ADESummer <- ACESummer
ADESummer_re <- ADESummer
AESummer_re <- AESummer
dropRcSummer_re <- dropRcSummer
dropRdSummer <- dropRcSummer
dropRdSummer_re <- dropRdSummer
...
load('/public/home/hongxming/bmi_ace/cpg_whr_all.Rdata')
cpg_whr_all <- cpg_whr_all[which(cpg_whr_all%in%CPG_all)]
CPG_all <- cpg_whr_all
...
for (i in 1:length(CPG_all)) {
    ...
    ###########################################################
    #set variables' parameters
    vars      <- c('WHR', vart)         # list of variables names
    ...
    svRd <- .2 # start value for shared env correlations
    ...
    pathD <- mxMatrix(type="Diag", nrow=nv, ncol=nv, free=TRUE, values=svPa, label=labDiag("d",nv), lbound=.0001, name="d")
    ...
    if((cor(deta[,vart_t1],deta[,'WHR_t1'])+cor(deta[,vart_t2],deta[,'WHR_t2']))>=0){
      pathRa <- mxMatrix(type="Stand", nrow=nv, ncol=nv, free=TRUE, values=svRa, label=labSdiag("ra",nv), lbound=0, ubound=1, name="Ra")
      pathRd <- mxMatrix(type="Stand", nrow=nv, ncol=nv, free=TRUE, values=svRd, label=labSdiag("rd",nv), lbound=0, ubound=1, name="Rd")
      ...
      pathRd <- mxMatrix(type="Stand", nrow=nv, ncol=nv, free=TRUE, values=svRd, label=labSdiag("rd",nv), lbound=-1, ubound=0, name="Rd")
    ...
    covD <- mxAlgebra(expression= d %*% (Rd) %*% t(d), name="D")
    ...
    covV  <- mxAlgebra(expression= A+D+E, name="V")
    ...
    ren <- mxAlgebra(expression = cov2cor(E), name = "ren")
    a2 <- mxAlgebra(expression = A/V, name = "a2")
    d2 <- mxAlgebra(expression = D/V, name = "d2")
    e2 <- mxAlgebra(expression = E/V, name = "e2")
    pa <- mxAlgebra(expression=sqrt(a2[1,1])*sqrt(a2[2,2])*Ra[1,2]/Rph[1,2],name = "pa")
    pd <- mxAlgebra(expression=sqrt(d2[1,1])*sqrt(d2[2,2])*Rd[1,2]/Rph[1,2],name = "pd")
    pe <- mxAlgebra(expression=sqrt(e2[1,1])*sqrt(e2[2,2])*Re[1,2]/Rph[1,2],name = "pe")
    
    ciADE <- mxCI(c("a[1,1]","a[2,2]","d[1,1]","d[2,2]","e[1,1]","e[2,2]","A[1,1]","A[2,1]","A[2,2]","D[1,1]","D[2,1]","D[2,2]",
                    "E[1,1]","E[2,1]","E[2,2]","a2[1,1]","a2[1,2]","a2[2,2]","d2[1,1]","d2[1,2]","d2[2,2]",
                    "e2[1,1]","e2[1,2]","e2[2,2]","Rph[1,2]","Ra[2,1]","Rd[2,1]","Re[2,1]","pa","pd","pe","ren[1,2]"))
    calc <- list(Rph, a2, d2, e2, pa, pd, pe, ren, ciADE)
    pars <- list(matI, matI2, matZ2)
    parsT <- list(meanZ)
    parsZ <- list(pathA, pathD, pathE, pathRa, pathRd, pathRe, covA, covD, covE, covV, invSD, pathB, invIB, covIV)
    ...
    multi <- mxFitFunctionMultigroup(c("MZ","DZ"))
    modelADE <- mxModel("ADE", pars, parsT, parsZ, modelT, multi, calc)
    fitADE <- mxTryHard(modelADE, extraTries=15, greenOK=TRUE, silent=TRUE, intervals = TRUE)
    mxCompare(fitSAT, c(fitADE))  ### 0.3961476
    summary(fitADE)   ### AIC:991.951   BIC:1037.005
    ADESumm <- summary(fitADE,verbose = T)
    ...
    # Run AE model
      dropD <- omxSetParameters(fitADE,labels=c("d11","d22"),free=F,values=0,name="dropD")
      
      AEfit <- omxSetParameters(dropD,labels=c("rd21"), free=F, values=0, name="AEfit")
      AEfit  <- mxTryHard(AEfit, intervals=T, extraTries = 15)
      # mxCompare(fitACE, c(dropCFit, AEfit))
      
      dropD1 <- omxSetParameters(fitADE,labels=c("d11"),free=F,values=0,name="dropD1")
      dropD2 <- omxSetParameters(fitADE,labels=c("d22"),free=F,values=0,name="dropD2")
      
      dropRd <- omxSetParameters(dropD1,labels=c("rd21"),free=F,values=0,name="dropRd")
      dropRdfit  <- mxTryHard(dropRd, intervals=T, extraTries = 15)
      dropRdSumm <- summary(dropRdfit,verbose = T)
      
      fitcompare <- mxCompare(fitADE, c(AEfit,dropRdfit))  # yes #
      mxCompare(fitSAT, c(fitADE))
      ...
      dropRdSumm <- summary(dropRdfit,verbose = T)
      dropRdSumm$CI[25:32, ]
      dropRdSumm$CIdetail[31:64, 1:3]
      
      ADESummer <- ACESummer_re
      AESummer <- AESummer_re
      dropRdSummer <- dropRcSummer_re
      
      if(is.null(ADESumm$CI[25,2])){
        fitADE <- mxTryHard(modelADE, extraTries=15, greenOK=TRUE, silent=TRUE, intervals = TRUE)
        ADESumm <- summary(fitADE,verbose = T)
      }
      ...
      if(!is.null(ADESumm$CI[25,2])){
        ADESumm$CI[7,1] <- ADESumm$CIdetail[13,3]
        ADESumm$CI[7,3] <- ADESumm$CIdetail[14,3]
        ADESumm$CI[8,1] <- ADESumm$CIdetail[15,3]
        ADESumm$CI[8,3] <- ADESumm$CIdetail[16,3]
        ADESumm$CI[9,1] <- ADESumm$CIdetail[17,3]
        ADESumm$CI[9,3] <- ADESumm$CIdetail[18,3]
        ADESumm$CI[10,1] <- ADESumm$CIdetail[19,3]
        ADESumm$CI[10,3] <- ADESumm$CIdetail[20,3]
        ADESumm$CI[11,1] <- ADESumm$CIdetail[21,3]
        ADESumm$CI[11,3] <- ADESumm$CIdetail[22,3]
        ADESumm$CI[12,1] <- ADESumm$CIdetail[23,3]
        ADESumm$CI[12,3] <- ADESumm$CIdetail[24,3]
        ADESumm$CI[13,1] <- ADESumm$CIdetail[25,3]
        ADESumm$CI[13,3] <- ADESumm$CIdetail[26,3]
        ADESumm$CI[14,1] <- ADESumm$CIdetail[27,3]
        ADESumm$CI[14,3] <- ADESumm$CIdetail[28,3]
        ADESumm$CI[15,1] <- ADESumm$CIdetail[29,3]
        ADESumm$CI[15,3] <- ADESumm$CIdetail[30,3]
        ADESumm$CI[25,1] <- ADESumm$CIdetail[49,3]
        ADESumm$CI[25,3] <- ADESumm$CIdetail[50,3]
        ADESumm$CI[26,1] <- ADESumm$CIdetail[51,3]
        ADESumm$CI[26,3] <- ADESumm$CIdetail[52,3]
        ADESumm$CI[27,1] <- ADESumm$CIdetail[53,3]
        ADESumm$CI[27,3] <- ADESumm$CIdetail[54,3]
        ADESumm$CI[28,1] <- ADESumm$CIdetail[55,3]
        ADESumm$CI[28,3] <- ADESumm$CIdetail[56,3]
        ADESumm$CI[29,1] <- ADESumm$CIdetail[57,3]
        ADESumm$CI[29,3] <- ADESumm$CIdetail[58,3]
        ADESumm$CI[30,1] <- ADESumm$CIdetail[59,3]
        ADESumm$CI[30,3] <- ADESumm$CIdetail[60,3]
        ADESumm$CI[31,1] <- ADESumm$CIdetail[61,3]
        ADESumm$CI[31,3] <- ADESumm$CIdetail[62,3]
        ADESumm$CI[32,1] <- ADESumm$CIdetail[63,3]
        ADESumm$CI[32,3] <- ADESumm$CIdetail[64,3]
        ACESummer[c(25:32,7:15),] <- ADESumm$CI[c(25:32,7:15),]
      }else{
      ...
      if(!is.null(dropRdSumm$CI[25,2])){
        dropRdSumm$CI[7,1] <- dropRdSumm$CIdetail[13,3]
        dropRdSumm$CI[7,3] <- dropRdSumm$CIdetail[14,3]
        dropRdSumm$CI[8,1] <- dropRdSumm$CIdetail[15,3]
        dropRdSumm$CI[8,3] <- dropRdSumm$CIdetail[16,3]
        dropRdSumm$CI[9,1] <- dropRdSumm$CIdetail[17,3]
        dropRdSumm$CI[9,3] <- dropRdSumm$CIdetail[18,3]
        dropRdSumm$CI[10,1] <- dropRdSumm$CIdetail[19,3]
        dropRdSumm$CI[10,3] <- dropRdSumm$CIdetail[20,3]
        dropRdSumm$CI[11,1] <- dropRdSumm$CIdetail[21,3]
        dropRdSumm$CI[11,3] <- dropRdSumm$CIdetail[22,3]
        dropRdSumm$CI[12,1] <- dropRdSumm$CIdetail[23,3]
        dropRdSumm$CI[12,3] <- dropRdSumm$CIdetail[24,3]
        dropRdSumm$CI[13,1] <- dropRdSumm$CIdetail[25,3]
        dropRdSumm$CI[13,3] <- dropRdSumm$CIdetail[26,3]
        dropRdSumm$CI[14,1] <- dropRdSumm$CIdetail[27,3]
        dropRdSumm$CI[14,3] <- dropRdSumm$CIdetail[28,3]
        dropRdSumm$CI[15,1] <- dropRdSumm$CIdetail[29,3]
        dropRdSumm$CI[15,3] <- dropRdSumm$CIdetail[30,3]
        dropRdSumm$CI[25,1] <- dropRdSumm$CIdetail[49,3]
        dropRdSumm$CI[25,3] <- dropRdSumm$CIdetail[50,3]
        dropRdSumm$CI[26,1] <- dropRdSumm$CIdetail[51,3]
        dropRdSumm$CI[26,3] <- dropRdSumm$CIdetail[52,3]
        dropRdSumm$CI[27,1] <- dropRdSumm$CIdetail[53,3]
        dropRdSumm$CI[27,3] <- dropRdSumm$CIdetail[54,3]
        dropRdSumm$CI[28,1] <- dropRdSumm$CIdetail[55,3]
        dropRdSumm$CI[28,3] <- dropRdSumm$CIdetail[56,3]
        dropRdSumm$CI[29,1] <- dropRdSumm$CIdetail[57,3]
        dropRdSumm$CI[29,3] <- dropRdSumm$CIdetail[58,3]
        dropRdSumm$CI[30,1] <- dropRdSumm$CIdetail[59,3]
        dropRdSumm$CI[30,3] <- dropRdSumm$CIdetail[60,3]
        dropRdSumm$CI[31,1] <- dropRdSumm$CIdetail[61,3]
        dropRdSumm$CI[31,3] <- dropRdSumm$CIdetail[62,3]
        dropRdSumm$CI[32,1] <- dropRdSumm$CIdetail[63,3]
        dropRdSumm$CI[32,3] <- dropRdSumm$CIdetail[64,3]
        dropRcSummer[c(25:32,7:15),] <- dropRdSumm$CI[c(25:32,7:15),]
      }else{
        dropRcSummer <- dropRcSummer_re
      }
    fitcompare <- as.data.frame(fitcompare)
    fitcompare$comparison[1] <- 'fitADE'
    lon <- omxSetParameters(model=get(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),labels="ra21",free=F,values=bi_whr_1_result[vart,'Ra'],name="lon")
    lon <- omxSetParameters(model=get(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),labels="re21",free=F,values=bi_whr_1_result[vart,'Re'],name="lon")
    lonfit <- mxTryHardOrdinal(lon,intervals=T,extraTries = 15)
    loncompare <- mxCompare(base=get(ifelse(is.na(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),'fitADE',fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))])),comparison=lonfit)
    lonresult[vart,'P_value'] <- loncompare[2,'p']

  }, error = function(e) {print(paste("error",i,sep=" "))})
}
write.csv(lonresult,file = '/public/home/hongxming/bmi_ace/bi_whr_results_2_ade_compare.csv')
