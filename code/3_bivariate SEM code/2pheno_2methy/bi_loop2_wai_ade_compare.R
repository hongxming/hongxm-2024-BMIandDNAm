...
PHENO_2$WAI_1 <- mean1
PHENO_2$WAI_2 <- mean2
PHENO_2 <- subset(PHENO_2,select = c(ID20,WAI_2))
deta <- merge(deta,PHENO_2,by.x='ID20_t1',by.y='ID20',all.x=T)
deta <- merge(deta,PHENO_2,by.x='ID20_t2',by.y='ID20',all.x=T)
deta$WAI_t1 <- deta$WAI_2.x
deta$WAI_t2 <- deta$WAI_2.y
deta_re <- deta
load('/public/home/hongxming/bmi_ace/result_frame.Rdata')
bi_wai_1_result <- read.csv('/public/home/hongxming/bmi_ace/bi_wai_1_result.csv')
rownames(bi_wai_1_result) <- bi_wai_1_result$CPG
...
load('/public/home/hongxming/bmi_ace/cpg_wai_all.Rdata')
cpg_wai_all <- cpg_wai_all[which(cpg_wai_all%in%CPG_all)]
CPG_all <- cpg_wai_all
...
for (i in 1:length(CPG_all)) {
    ...
    ###########################################################
    #set variables' parameters
    vars      <- c('WAI', vart)         # list of variables names
    ...
    if((cor(deta[,vart_t1],deta[,'WAI_t1'])+cor(deta[,vart_t2],deta[,'WAI_t2']))>=0){
    ...
    lon <- omxSetParameters(model=get(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),labels="ra21",free=F,values=bi_wai_1_result[vart,'Ra'],name="lon")
    lon <- omxSetParameters(model=get(fitcompare$comparison[which(fitcompare$AIC==min(fitcompare$AIC))]),labels="re21",free=F,values=bi_wai_1_result[vart,'Re'],name="lon")
...
write.csv(lonresult,file = '/public/home/hongxming/bmi_ace/bi_wai_results_2_ade_compare.csv')
