...
load('/public/home/hongxming/data_longi/deta_longi1.Rdata')
mean1 <- c()  #mean sbp
mean2 <- c()  #mean dbp
for (i in 1:nrow(PHENO_1)){
  if(is.na(PHENO_1$WAI1_1[i]) == T){
    mean1 <- c(mean1, NA)
  }else{
    d <- c(PHENO_1$WAI1_1[i],PHENO_1$WAI2_1[i],PHENO_1$WAI3_1[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
  ...
  if(is.na(PHENO_1$WAI1_1[i]) == T){
    ...
    d <- c(PHENO_1$WAI1_2[i], PHENO_1$WAI2_2[i], PHENO_1$WAI3_2[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
...
mean2 <- c()  #mean dbp
for (i in 1:nrow(PHENO_1)){
  if(is.na(PHENO_1$HIP1_1[i]) == T){
    mean1 <- c(mean1, NA)
  }else{
    d <- c(PHENO_1$HIP1_1[i],PHENO_1$HIP2_1[i],PHENO_1$HIP3_1[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
  ...
  if(is.na(PHENO_1$HIP1_1[i]) == T){
    ...
    d <- c(PHENO_1$HIP1_2[i], PHENO_1$HIP2_2[i], PHENO_1$HIP3_2[i])
    e <- na.omit(d)
    a <- max(e)
    b <- min(e)
    c <- sum(e) - a - b
...
PHENO_1$HIP_1 <- mean1
PHENO_1$HIP_2 <- mean2
PHENO_1$WHR_1 <- PHENO_1$WAI_1/PHENO_1$HIP_1
PHENO_1 <- subset(PHENO_1,select = c(ID20,WHR_1))
deta <- merge(deta,PHENO_1,by.x='ID20_t1',by.y='ID20',all.x=T)
deta <- merge(deta,PHENO_1,by.x='ID20_t2',by.y='ID20',all.x=T)
deta$WHR_t1 <- deta$WHR_1.x
deta$WHR_t2 <- deta$WHR_1.y
deta_re <- deta
...
for (i in 1:length(CPG_all)) {
    ...
    # Run AE model
      ...
      dropRdfit <- omxSetParameters(dropD1,labels=c("rd21"),free=F,values=0,name="dropRd")
      dropRdfit  <- mxTryHard(dropRdfit, intervals=T, extraTries = 15)
...
write.csv(result,file = '/public/home/hongxming/bmi_ace/bi_whr_results_1_ade.csv')
